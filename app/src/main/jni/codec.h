...
using namespace std;
 ...
 int codec_base64_encode(const unsigned char *bs,size_t len ,std::string& finalDst);//声明 static 又不给定义，肯定找不到
 int codec_base64_encode(const vector<unsigned char> &src,string &dest);
 int codec_base64_decode(const char *cs,size_t decryptedLen,vector<unsigned char> finalDst);//函数声明时默认带extern，不用显式，也可以
